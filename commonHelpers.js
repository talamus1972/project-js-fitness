import{h as v,b as $,a as k,i as c,c as S,o as E,n as P}from"./assets/change-theme-05eae00a.js";import"./assets/vendor-0a7943b3.js";const M="/project-js-fitness/assets/favorite-dumbbell-325968d2.png",i=document.querySelector(".favorites-exercise-container"),m=document.querySelector(".render-btn-list-pagination"),b="favorites";let s=null;const I=getComputedStyle(document.querySelector("body")).width,q=parseInt(I)<768?8:9;let f=1;const u=8,_=async t=>{try{return(await k.get(`/exercises/${t}`)).data}catch(a){console.log(a)}},T=()=>{try{s=JSON.parse(localStorage.getItem(b))}catch(t){console.log(t)}return s},B=()=>{i.innerHTML="";const t=`<div class="favorites-exercise-not-found-container">
        <img
        ...
        </li>`,"");i.insertAdjacentHTML("beforeend",a)};async function p(){if(T(),s!==null&&q===8&&s.length>8)H(f);else if(s!==null&&s.length!==0){const t=await Promise.all(s.map(a=>_(a)));y(t),v(m)}else B()}p();const C=t=>{t.preventDefault();const a=t.target;if(t.target.closest(".icon-basket ")){const o=a.classList[1],r=s.filter(n=>n!==o);localStorage.setItem(b,JSON.stringify(r)),p()}};i.addEventListener("click",C);async function H(t){const a=Math.ceil(s.length/u);$(a,t);const e=(t-1)*u,o=e+u,r=s.slice(e,o),n=await Promise.all(r.map(d=>_(d)));y(n)}m.addEventListener("click",t=>{f=Number(t.target.innerHTML),p()});const j=document.querySelector(".favorites-exercise-container"),l=document.querySelector(".modal-trane-background"),g=()=>{l.classList.remove("modal-trane-background-active"),document.removeEventListener("click",h),document.removeEventListener("keydown",w)},h=t=>{t.target===l&&g()},w=t=>{t.key==="Escape"&&g()},A=async t=>{const e=(await k.get(`exercises/${t}`)).data,o=`<div class="modal-trane">
  ...
  </div>`;l.innerHTML=o,document.querySelector(".modal-trane-btn-close").addEventListener("click",g),document.querySelector(".modal-trane-btn-rating").addEventListener("click",E)};j.addEventListener("click",t=>{t.target.classList.contains("workout-card__link-start")&&(A(t.target.dataset.id),l.classList.add("modal-trane-background-active"),document.addEventListener("click",h),document.addEventListener("keydown",w))});P();
